...
# Define model
model = Sequential()
model.add(Conv2D(8,3,activation='relu',padding='valid',kernel_initializer='he_normal',kernel_regularizer='l2',input_shape=X_tr.shape[1:]))
model.add(Dropout(.3))
...
model.add(Dense(num_channels,activation='relu',kernel_initializer='he_normal',kernel_regularizer='l2',use_bias=False))

# Save model configuration
model.summary()
model_json = model.to_json()

print('Training shared model for %d channels -----------------'%Y_tr.shape[1])
model = model_from_json(model_json)
model.compile(loss='mse',optimizer='rmsprop')

filepath = 'model-shared.hdf5'
...
model.fit(X_tr,Y_tr,batch_size=64,epochs=20,validation_split=.2,verbose=1,callbacks=callbacks)
model.load_weights(filepath)

corrs = []
Z_te = model.predict(X_te)
for ch in range(num_channels):
    corr = np.corrcoef(Z_te[:,ch].T, Y_te[:,ch].T)[0,1]
    corrs.append(corr)
print(corrs)
